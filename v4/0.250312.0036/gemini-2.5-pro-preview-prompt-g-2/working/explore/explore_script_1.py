# explore_script_1.py
# This script loads the NWB file and plots the first current clamp response
# (current_clamp-response-01-ch-0) and its corresponding stimulus
# (stimulus-01-ch-0).
...
# Get the first current clamp response and stimulus
response_series = nwb.acquisition["current_clamp-response-01-ch-0"]
stimulus_series = nwb.stimulus["stimulus-01-ch-0"]

# Get data (load a subset to avoid loading too much data)
num_points_to_plot = 20000  # Plot 1 second of data (rate is 20000 Hz)
...
plt.tight_layout()
plt.savefig("response_stimulus_plot.png") # Save relative to script execution directory

print("Plotted current_clamp-response-01-ch-0 and stimulus-01-ch-0 to response_stimulus_plot.png")
