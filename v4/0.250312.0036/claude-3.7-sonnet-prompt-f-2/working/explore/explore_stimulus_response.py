...
# We'll look at recordings 1, 20, 50, 100, and 200 to get a spread
recording_indices = [1, 20, 50, 100, 200]

plt.figure(figsize=(15, 12))

for i, idx in enumerate(recording_indices):
    # Get the response and stimulus data
    response_key = f"current_clamp-response-{idx:03d}-ch-0"
    stimulus_key = f"stimulus-{idx:03d}-ch-0"
...
response_key = "current_clamp-response-001-ch-0"
stimulus_key = "stimulus-001-ch-0"
...
for i, idx in enumerate([1, 50, 100, 150, 200, 250]):
    stimulus_key = f"stimulus-{idx:03d}-ch-0"
    stimulus = nwb.stimulus[stimulus_key]
    stimulus_data = stimulus.data[:] * stimulus.conversion * 1e12  # Convert to pA
...
# Compare the first 10 and last 10 recordings
plt.figure(figsize=(15, 12))
...
last_indices = sorted([int(k.split('-')[2]) for k in response_keys])[-5:]
